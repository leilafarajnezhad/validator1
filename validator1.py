...
def name_object(name,en_name,fa_name):
    if re.match(r"^[A-Z][a-z]+$", name):
            if type(fa_name) == str and re.match('^[\sآ-ی][3,30]$', fa_name):
...
def get_data():
      ...
      if code_object (code) and\
         ...
         product={"code_object":code_object, "name_object":name_object, "brand_object":brand_object, "price_object":price_object}
